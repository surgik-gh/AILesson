...
export class OpenRouterService {
  ...
  private async makeRequest(
    ...
    try {
      ...
      console.error(`Request failed with model ${model}:`, error);

      // Retry with same model
      if (retryCount < maxRetries - 1) {
        console.log(`Retrying request (attempt ${retryCount + 2}/${maxRetries})...`);
        await new Promise((resolve) => setTimeout(resolve, 1000 * (retryCount + 1)));
        return this.makeRequest(prompt, modelIndex, retryCount + 1);
      }

      // Try next model in fallback chain
      if (modelIndex < this.freeModels.length - 1) {
        console.log(`Falling back to next model: ${this.freeModels[modelIndex + 1]}`);
        return this.makeRequest(prompt, modelIndex + 1, 0);
      }

      // All retries and fallbacks exhausted
      throw new Error(
        `Failed to generate response after ${maxRetries} retries and ${this.freeModels.length} models`
      );
    }
  }
...
Make the expert feel unique and tailored to the student's preferences. Use Russian language for all text fields.`;
    ...
    try {
      ...
      // Return a default expert as fallback
      return {
...
- Order questions from 1 to N sequentially`;
    ...
    try {
      ...
      // Re-throw the original error to preserve error messages for testing
      throw error;
    }
  }
